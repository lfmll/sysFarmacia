...
class emisionIndividualTest extends TestCase
{
    ...
    public function testEmisionIndividual1()
    {
        ...
        if ($clienteFacturacion->verificarComunicacion()->return->transaccion == "true")
        {
            ...
            for ($i=0; $i < 80; $i++) { 
                ...
                $precio_compra = $faker->randomFloat(2,0.5,10);
                $precio_venta = round($precio_compra * 1.10,2);
                $lote = Lote::create([
                    'numero' => $faker->bothify('Lote-####'),
                    'cantidad' => 200,
                ...
                $medicamento->stock += $lote->cantidad;
                $medicamento->save();
                //Cliente
                ...
                $cuf = Factura::generarCUF($empresa->nit,
                                                $agencia->codigo,
                                                $fecha_emision,
                                                $empresa->modalidad,
                ...
                if (empty($msjError)) {
                    ...
                    if ($responseRecepcionFactura == "VALIDADA") {
                        ...
                        $this->fail("Factura no validada: {$responseRecepcionFactura}");                         
                    }                    
                } else {
                    $errores[] = $msjError;
                    $this->fail("Error de validación XML: {$msjError}");
                }
            }            
        } else {
    ...
    public function testEmisionIndividual2()
    {
        // codigoAmbiente = 2
        // codigoEmision = 1
        // archivo = su cadena de archivo
        ...
        $empresa = Empresa::first();
        $agencia = Agencia::where('codigo',0)->first();
        $puntoVenta = PuntoVenta::where('codigo',0)->first();
        $cuis = Cuis::where('codigo_cuis',"718E252F")
    ...
    public function testEmisionIndividual3()
    {
        // codigoAmbiente = 2
        // codigoEmision = 1
        // archivo = su cadena de archivo
        ...
        $agencia = Agencia::where('codigo',0)->first();
        $puntoVenta = PuntoVenta::where('codigo',1)->first();
        $cuis = Cuis::where('agencia_id',$agencia->id)
                    ->where('punto_venta_id',$puntoVenta->id)
                    ->where('estado','A')
                    ->first();
        $cufd = Cufd::where('estado','A')
        ...
        if ($clienteFacturacion->verificarComunicacion()->return->transaccion == "true")
        {
            ...
            for ($i=0; $i < 1; $i++) { 
                ...
                $cuf = Factura::generarCUF($empresa->nit,
                                                ...
                                                $empresa->modalidad,
                                                1, //Emision Online=1, Offline=2
                                                3, //Tipo Factura documento=1, documento sin valor fiscal=2, documento ajuste=3
                                                24, //Tipo Documento Sector factura compra venta=1, nota credito=2, nota debito=3
                                                Factura::count()+1, //Numero de factura
                                                $puntoVenta->codigo    //pos
                                                );
                $factura->cufd = $cufd->codigo_cufd;
                ...
                $factura->leyenda = 'Ley N° 453: El proveedor debe entregar al consumidor la factura o el comprobante de venta';
                $factura->usuario = 'Luis Fernando';
                $factura->codigoDocumentoSector = 24;
                $factura->estado = 'RECEPCION PENDIENTE';
                $factura->venta_id = $venta->id;
                $factura->save();
                ...
                if (empty($msjError)) {
                    ...
                    $errores[] = $msjError;
                    $this->fail("Error de validación XML: ".json_encode($msjError));
                }
            }            
        } else {
            $errores[] = "Error de comunicacion con SIAT";
